...
import static org.lilbrocodes.composer_reloaded.api.v1.overlay.Overlays.*;
...
public class OverlayCommand extends ComposerCommand {
                        ...
                        .then(
                                argument("visible", BoolArgumentType.bool()).then(
                                        argument("queue", BoolArgumentType.bool()).executes(ctx -> {
                                            ...
                                            if (visible && queue) return success(ctx, Text.translatable("composer_reloaded.overlay.cleared_all"));
                                            else if (visible) return success(ctx, Text.translatable("composer_reloaded.overlay.cleared_all_visible"));
                                            else return success(ctx, Text.translatable("composer_reloaded.overlay.cleared_all_queued"));
                                        })
                                )
                        )
        );
    ...
    private int clear(CommandContext<ServerCommandSource> ctx) {
        try {
            ...
            boolean queue = ctx.getArgument("queue", Boolean.class);
            ServerPlayNetworking.send(player, new ClearOverlaysPayload(visible, queue));
            if (visible && queue) return success(ctx, Text.translatable("composer_reloaded.overlay.cleared_all_for", player.getName()));
            else if (visible) return success(ctx, Text.translatable("composer_reloaded.overlay.cleared_all_visible_for", player.getName()));
            else return success(ctx, Text.translatable("composer_reloaded.overlay.cleared_all_queued_for", player.getName()));
        } catch (Exception e) {
            return error(ctx, Text.translatable("composer_reloaded.command.exception.player_not_found"));
        }
    }
    ...
    private int textureScaleDuration(CommandContext<ServerCommandSource> ctx) {
        try {
            ...
            return feedback(ctx, Text.translatable(
                    "composer_reloaded.overlay.sent_texture_scale_duration",
                    texture.toString(),
                    scale,
                    duration
            ));
        } catch (CommandSyntaxException e) {
            return error(ctx, Text.translatable("composer_reloaded.command.exception.no_players_found"));
        } catch (Exception e) {
            return error(ctx, Text.translatable("composer_reloaded.overlay.invalid_texture"));
        }
    }
    ...
    private int textureScaleFade(CommandContext<ServerCommandSource> ctx) {
        try {
            ...
            return feedback(ctx, Text.translatable(
                    "composer_reloaded.overlay.sent_texture_scale_fade",
                    texture.toString(),
                    scale,
                    enter,
                    hold,
    ...
    private int textureDuration(CommandContext<ServerCommandSource> ctx) {
        try {
            ...
            return feedback(ctx, Text.translatable(
                    "composer_reloaded.overlay.sent_texture_duration",
                    texture.toString(),
                    duration
            ));
        } catch (CommandSyntaxException e) {
    ...
    private int textureFade(CommandContext<ServerCommandSource> ctx) {
        try {
            ...
            return feedback(ctx, Text.translatable(
                    "composer_reloaded.overlay.sent_texture_fade",
                    texture.toString(),
                    enter,
                    hold,
                    exit
    ...
    private int textScaleDuration(CommandContext<ServerCommandSource> ctx) {
        try {
            ...
            return feedback(ctx, Text.translatable(
                    "composer_reloaded.overlay.sent_text_scale_duration",
                    text,
    ...
    private int textScaleFade(CommandContext<ServerCommandSource> ctx) {
        try {
            ...
            return feedback(ctx, Text.translatable(
                    "composer_reloaded.overlay.sent_text_scale_fade",
                    text,
                    String.format("#%06X", color & 0xFFFFFF),
                    scale,
                    enter,
    ...
    private int textDuration(CommandContext<ServerCommandSource> ctx) {
        try {
            ...
            return feedback(ctx, Text.translatable(
                    "composer_reloaded.overlay.sent_text_duration",
                    text,
                    String.format("#%06X", color & 0xFFFFFF),
                    duration
            ));
    ...
    private int textFade(CommandContext<ServerCommandSource> ctx) {
        try {
            ...
            return feedback(ctx, Text.translatable(
                    "composer_reloaded.overlay.sent_text_fade",
                    text,
                    String.format("#%06X", color & 0xFFFFFF),
                    enter,
                    hold,
    ...
    protected Text buildPrefix() {
        return wrapBrackets(createGradient(Text.translatable("composer_reloaded.overlay.prefix"), 0xffaa00, 0xffff55));
    }
}
